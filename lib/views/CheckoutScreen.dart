...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<CheckoutViewModel>(builder: (context, vm, child) {
          ...
          child: Column(
            ...
            children: [
              ...
              _buildSectionHeader("Payment Method", onChange: () async {
                final selected = await AppRoutes.navigateToPaymentMethods(
                    context, vm.paymentMethod);
              ...
              Container(
                ...
                child: Row(
                  children: [
                    Icon(
                        vm.paymentMethod.contains("Cash")
                            ? Icons.handshake
                            : Icons.credit_card,
                        color: vm.paymentMethod.contains("Cash")
                            ? Colors.green
                            : Colors.blue,
                        size: 30),
              ...
              const SizedBox(height: 25),
              const Text("Tip Rider",
                  style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const SizedBox(height: 5),
              const Text("All tips go directly to the delivery rider.",
                  ...
                  children: [200, 500, 1000, 1500, 2000].map((amount) {
                    ...
                    return Padding(
                      ...
                      child: ChoiceChip(
                        ...
                        onSelected: (selected) =>
                            vm.setTip(selected ? amount.toDouble() : 0.0),
                      ),
                    );
                  }).toList(),
                ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    _buildSummaryRow("Cost of total items:",
                        "₦${vm.subtotal.toStringAsFixed(0)}"),
                    const SizedBox(height: 12),
                    _buildSummaryRow("Delivery Fee:",
                        "₦${vm.deliveryFee.toStringAsFixed(0)}"),
                    const SizedBox(height: 12),
                    _buildSummaryRow(
                        "Tip:", "₦${vm.tipAmount.toStringAsFixed(0)}"),
                    const SizedBox(height: 12),
                    _buildSummaryRow(
                        "Discount:", "- ₦${vm.discount.toStringAsFixed(0)}"),
                    const Divider(
                        height: 30, color: Colors.white, thickness: 2),
                    Row(
